...
我们知道，父进程的栈原本是“可写”的（假设父进程的栈所在的 page 是 *Page_A*），当`duppage`把父进程的栈的写权限拿掉之后会发生什么？届时函数执行过程中的栈操作将触发父进程调用`fork`过程中的第一次 #PF，从而执行第一次 Copy-on-Write，*Page_A* 就会有一个可写的副本 *Page_A'*。由于子进程的栈也是映射到 *Page_A*，所以当子进程执行栈操作指令时也会触发 #PF，然后执行 Copy-on-Write 的时候又会产生 *Page_A* 的一个可写副本 *Page_A''*。最后 *Page_A* 引用计数为0就被回收了。
...
考虑以下情形：在父进程地址空间内`addr`指向 Page A, 如果它正好是当前函数调用过程中使用的栈, 父进程先把自己对它的权限变成 COW, 那么在发起下次`sys_page_map`调用的时候就会因为函数传参触发#PF, 按照 COW 流程就会在父进程地址空间内分配一个 Page A 的可写副本 Page A', `addr`更改为指向 Page A'. 当调用`sys_page_map`将父进程地址空间内`addr`指向的页面映射给子进程时, 实际就是在子进程地址空间内将`addr`映射到 Page A'. 此时的情形是, 父子进程地址空间内`addr`都指向 Page A', 但父进程对其有写权限, 子进程则是 COW 权限, 那么父进程后续在 Page A' 上的修改对子进程是可见的. 如果在子进程地址空间内, `esp`指向的内存被父进程写为0, `ret`时就会触发异常.

如果先将`addr`指向的 Page A 映射给子进程, 再把父进程对 Page A 的权限改为 COW, 那么后续父进程写 Page A 时就会触发 COW 流程, 创建可写副本 Page A', 并将`addr`映射到 Page A'. 此时父子进程地址空间内`addr`指向不同的页面, 符合预期.
