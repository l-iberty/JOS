...
static void check_regs(struct regs *a, const char *an, struct regs *b, const char *bn, const char *testname) {
  int mismatch = 0;
...
#define CHECK(name, field)                              \
  do {                                                  \
    printf("%s %08x %08x ", #name, a->field, b->field); \
    if (a->field == b->field)                           \
      printf("OK\n");                                   \
    else {                                              \
      printf("MISMATCH\n");                             \
...
static void pgfault(struct UTrapframe *utf) {
  ...
  if (utf->utf_fault_va != (uint32_t)UTEMP)
    panic("pgfault expected at UTEMP, got 0x%08x (eip %08x)", utf->utf_fault_va, utf->utf_eip);
  ...
  if ((r = sys_page_alloc(0, UTEMP, PTE_U | PTE_P | PTE_W)) < 0) panic("sys_page_alloc: %e", r);
}
